...
function ensureArgs(command, options) {
  let cwd = _optionalChain([options, 'optionalAccess', _ => _.cwd]) || process.cwd();
  if (process.env.PRIORITY_ADB_MODULE_TYPE === "custom" /* Custom */ || process.env.PRIORITY_ADB_MODULE_TYPE === "internal" /* Internal */ || !isSystemAdbAvailable()) {
  ...
  const res = [
    command,
    {
      ...options,
      cwd,
      timeout: _optionalChain([options, 'optionalAccess', _2 => _2.timeout]) || TIMEOUT
    }
  ];
  return res;
}
...
function execAdbCmdAsync(command, options) {
  return new Promise(async (resolve2, reject) => {
    _child_process.exec.call(void 0, ...ensureArgs(command, options), (err, stdout, stderr) => {
      if (err)
        return reject(err);
